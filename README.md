I am a computational astrophysicist researching dark matter — what it is, and how it shapes galaxies — mostly by using stellar streams to constrain their gravitational potentials, in the Milky Way and far beyond it with large-scale surveys.

When a star cluster is torn apart by its host galaxy, the debris can trace a long, thin stream. Properties of the stream — its path, shape, width, etc — are all sensitive to properties of the host, including its dark matter. I develop and apply novel methods to study these streams and learn about the dark matter; I also build much of the software powering these methods and analyses. I am also a core developer of Astropy, helping to power all of astronomy.

### 🔗 Links

✉️ starkman [at] mit [dot] edu &nbsp;|&nbsp; 🌐 [nstarkman.space](https://nstarkman.space)

<img src="https://raw.githubusercontent.com/nstarman/nstarman/main/assets/ads.svg" alt="ADS" width="16" height="16"> [**Publications**](https://ui.adsabs.harvard.edu/search/q=author%3A%22Starkman%2C%20Nathaniel%22&sort=date%20desc) &nbsp;|&nbsp; <img src="https://raw.githubusercontent.com/nstarman/nstarman/main/assets/arxiv.svg" alt="arXiv" width="16" height="16"> [**arXiv**](https://arxiv.org/a/starkman_n_1) &nbsp;|&nbsp; <img src="https://raw.githubusercontent.com/nstarman/nstarman/main/assets/zenodo.svg" alt="Zenodo" width="16" height="16"> [**Zenodo**](https://zenodo.org/search?q=%22Starkman%2C%20Nathaniel%22) &nbsp;|&nbsp; <img src="https://raw.githubusercontent.com/nstarman/nstarman/main/assets/orcid.svg" alt="ORCID" width="16" height="16"> [**ORCID** 0000-0003-3954-3291](https://orcid.org/0000-0003-3954-3291) &nbsp;|&nbsp; 🎓 [**Google Scholar**](https://scholar.google.com/citations?user=WxfvklgAAAAJ) &nbsp;|&nbsp; 🐙 [**GitHub**](https://github.com/nstarman)

### 📄 Select Publications

> 📄 paper · 🔨 paper repo · 💻 code · 📖 docs · 🗄️ data

1. Euclid Collaboration, **N. Starkman**, J. Nibauer, S. Pearson, S. Wu, et al. (2026). *Euclid Quick Data Release (Q1): The geometry of dark matter halos from extragalactic streams.* Astronomy & Astrophysics. *submitted.*&nbsp;&nbsp;[📜](https://arxiv.org/abs/2606.21774 "preprint") [📄](https://ui.adsabs.harvard.edu/abs/2026arXiv260621774E/abstract "ADS")
1. C. Myers, **N. Starkman**, L. Necib (2026). *Physics-Informed Neural Networks for Modeling Galactic Gravitational Potentials.* Monthly Notices of the Royal Astronomical Society. *submitted.*&nbsp;&nbsp;[💻](https://github.com/charlottemyers/galactoPINNs "code") [📄](https://ui.adsabs.harvard.edu/abs/2026arXiv260618386M/abstract "ADS")
1. **N. Starkman**, A. M. Price-Whelan, J. Nibauer (2025). *unxt: A Python package for unit-aware computing with JAX.* Journal of Open Source Software 10, 7771.&nbsp;&nbsp;[📄](https://doi.org/10.21105/joss.07771 "paper") [💻](https://github.com/GalacticDynamics/unxt "code") [📖](https://unxt.readthedocs.io/en/stable/ "docs")
1. **N. Starkman**, J. Nibauer, J. Bovy, J. Webb, et al. (2025). *Stream Members Only: Data-Driven Characterization of Stellar Streams with Mixture Density Networks.* The Astrophysical Journal 980, 253.&nbsp;&nbsp;[📄](https://iopscience.iop.org/article/10.3847/1538-4357/ad94f2 "paper") [🔨](https://github.com/nstarman/stellar_stream_density_ml_paper "paper repo") [🗄️](https://zenodo.org/records/10211410 "data")
1. **N. Starkman**, A. Kosowsky, G. Starkman (2024). *Angular Correlations of Cosmic Microwave Background Spectrum Distortions from Photon Diffusion.* Monthly Notices of the Royal Astronomical Society 529, 2274.&nbsp;&nbsp;[📄](https://academic.oup.com/mnras/article/529/3/2274/7630233 "paper") [🔨](https://github.com/nstarman/Temperature-Diffusion-Spectral-Distortion-Paper "paper repo") [🗄️](https://zenodo.org/record/8400583 "data")

➡️ Full publication list on [**ADS**](https://ui.adsabs.harvard.edu/search/q=author%3A%22Starkman%2C%20Nathaniel%22&sort=date%20desc).

### 💻 Software

<p>
<a href="https://github.com/astropy/astropy"><picture><source media="(prefers-color-scheme: dark)" srcset="assets/cards/astropy-dark.svg"><img src="assets/cards/astropy-light.svg" alt="astropy" width="400"></picture></a>
<a href="https://github.com/GalacticDynamics/coordinax"><picture><source media="(prefers-color-scheme: dark)" srcset="assets/cards/coordinax-dark.svg"><img src="assets/cards/coordinax-light.svg" alt="coordinax" width="400"></picture></a>
<a href="https://github.com/GalacticDynamics/galax"><picture><source media="(prefers-color-scheme: dark)" srcset="assets/cards/galax-dark.svg"><img src="assets/cards/galax-light.svg" alt="galax" width="400"></picture></a>
<a href="https://github.com/beartype/plum"><picture><source media="(prefers-color-scheme: dark)" srcset="assets/cards/plum-dark.svg"><img src="assets/cards/plum-light.svg" alt="plum" width="400"></picture></a>
<a href="https://github.com/nstarman/quax"><picture><source media="(prefers-color-scheme: dark)" srcset="assets/cards/quax-dark.svg"><img src="assets/cards/quax-light.svg" alt="quax" width="400"></picture></a>
<a href="https://github.com/GalacticDynamics/unxt"><picture><source media="(prefers-color-scheme: dark)" srcset="assets/cards/unxt-dark.svg"><img src="assets/cards/unxt-light.svg" alt="unxt" width="400"></picture></a>
</p>

<details>
<summary><i>Click to expand:</i> the rest of the ecosystem.</summary>


- [**cosmology.api**](https://pypi.org/project/cosmology.api/): A standard interface for cosmology libraries [💻](https://pypi.org/project/cosmology.api/ "code") [📖](https://cosmology.readthedocs.io/en/latest/ "docs") [🗄️](https://doi.org/10.5281/zenodo.8331505 "data")
- [**diffraxtra**](https://github.com/GalacticDynamics/diffraxtra): Extras for Diffrax [💻](https://github.com/GalacticDynamics/diffraxtra "code")
- [**galactic_dynamics_interoperability**](https://github.com/GalacticDynamics/galactic_dynamics_interoperability): Interoperability between galactic dynamics libraries [💻](https://github.com/GalacticDynamics/galactic_dynamics_interoperability "code")
- [**is_annotated**](https://github.com/GalacticDynamics/is_annotated): Check whether a type hint is Annotated [💻](https://github.com/GalacticDynamics/is_annotated "code")
- [**jaxmore**](https://github.com/GalacticDynamics/jaxmore): There's more to JAX [💻](https://github.com/GalacticDynamics/jaxmore "code")
- [**macro_lightning**](https://pypi.org/project/macro-lightning/): Constrain macroscopic dark matter with lightning [💻](https://pypi.org/project/macro-lightning/ "code") [📖](https://macro-lightning.readthedocs.io/en/latest/ "docs") [🗄️](https://zenodo.org/record/4558581 "data")
- [**mvgkde**](https://github.com/nstarman/mvgkde): Multivariate Gaussian KDE [💻](https://github.com/nstarman/mvgkde "code")
- [**optional_dependencies**](https://github.com/GalacticDynamics/optional_dependencies): Construct checks for optional dependencies [💻](https://github.com/GalacticDynamics/optional_dependencies "code")
- [**phasecurvefit**](https://github.com/GalacticDynamics/phasecurvefit): Paths through phase space [💻](https://github.com/GalacticDynamics/phasecurvefit "code")
- [**plotting_backends**](https://github.com/GalacticDynamics/plotting_backends): Dispatch over plotting backends [💻](https://github.com/GalacticDynamics/plotting_backends "code")
- [**quax-blocks**](https://github.com/GalacticDynamics/quax-blocks): Building blocks for quax classes [💻](https://github.com/GalacticDynamics/quax-blocks "code")
- [**quaxed**](https://github.com/GalacticDynamics/quaxed): Pre-quaxed libraries [💻](https://github.com/GalacticDynamics/quaxed "code")
- [**trackstream**](https://github.com/nstarman/trackstream): Stellar stream tracks [💻](https://github.com/nstarman/trackstream "code")

</details>
